...
//#define REGISTER_SERIALIZE_HANDLER reflect::Serializer::register_handler<type>()
//#define REGISTER_DESERIALIZE_HANDLER reflect::Deserializer::register_handler<type>()


#define REGISTER_SERIALIZE_HANDLER(...) proto::Serializer::RegisterHandler<__VA_ARGS__>()
#define REGISTER_DESERIALIZE_HANDLER(...) proto::Deserializer::RegisterHandler<__VA_ARGS__>()
...
namespace reflect {
  ...
  template<typename T>
  constexpr inline const auto has_begin_v = std::experimental::is_detected_v<has_begin_t, T>;
  template<typename D>
  using has_end_t = decltype(std::declval<D &>().end());
  template<typename T>
  constexpr inline const auto has_end_v = std::experimental::is_detected_v<has_end_t, T>;

  template<typename T>
  constexpr inline const auto is_iterable_v = has_begin_v<T> && has_end_v<T>;
  // 判断是否pair
  template<typename D>
  using has_first_t = decltype(std::declval<D &>().first);
  template<typename T>
  constexpr inline const auto has_first_v = std::experimental::is_detected_v<has_first_t, T>;
  template<typename D>
  using has_second_t = decltype(std::declval<D &>().second);
  template<typename T>
  constexpr inline const auto has_second_v = std::experimental::is_detected_v<has_second_t, T>;
  template<typename T>
  constexpr inline const auto is_pair_v = has_first_v<T> && has_second_v<T>;
  // 判断是否reflect类型
  template<typename D>
  using has_get_field_info_t = decltype(std::declval<D &>().get_field_info_vec());

  template<typename T>
  constexpr inline const auto is_reflect_v = std::experimental::is_detected_v<has_get_field_info_t, T>;
  template<typename D>
...
public: \
 ...
 private:inline static const auto _unique_var = (REGISTER_SERIALIZE_HANDLER(type),0);\
 inline static const auto _unique_var = (REGISTER_DESERIALIZE_HANDLER(type), 0)
...
#define reflect_field(type, name, ...)\
 ...
 inline static const auto _unique_var __attribute__((used)) = (REGISTER_SERIALIZE_HANDLER(type), 0);\
 inline static const auto _unique_var __attribute__((used)) = (REGISTER_DESERIALIZE_HANDLER(type), 0);
